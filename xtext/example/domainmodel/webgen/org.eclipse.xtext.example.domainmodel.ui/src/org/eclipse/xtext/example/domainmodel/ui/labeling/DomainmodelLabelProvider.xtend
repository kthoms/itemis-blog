...
class DomainmodelLabelProvider extends XbaseLabelProvider {
	...
	def protected void append(StringBuilder builder, JvmTypeReference typeRef) {
		if (typeRef instanceof JvmParameterizedTypeReference) {
			...
			append(builder, type)
			var EList<JvmTypeReference> arguments = ((typeRef as JvmParameterizedTypeReference)).getArguments()
			...
			builder.append("?")
			var Iterator<JvmTypeConstraint> iterator = ((typeRef as JvmWildcardTypeReference)).getConstraints().
				iterator()
			while (iterator.hasNext()) {
	...
	def protected void append(StringBuilder builder, JvmType type) {
		if (type instanceof JvmArrayType) {
			append(builder, ((type as JvmArrayType)).getComponentType())
			builder.append("[]")
		} else {
